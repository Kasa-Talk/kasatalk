    ...
    <div className="px-4">
      <div className="bg-white max-w-6xl mx-auto p-4 my-6 border rounded-lg min-h-[35rem]">
        <div className="flex max-w-4xl mx-auto gap-4 mt-6">
          <input type="text" onChange={(e) => setSearchValue(e.target.value)} className="border rounded-full py-[0.65rem] px-4 w-full focus:border-primary outline-none" placeholder="Cari kata ex: apa, dimana" />
        ...
        {!isLoading && words.length > 0 && (
          <ul className="mt-10">
            {words.map(({ title, description }, index) => (
              <li key={index}>
